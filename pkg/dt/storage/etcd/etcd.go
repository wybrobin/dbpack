...
func (s *store) AddBranchSession(ctx context.Context, branchSession *api.BranchSession) error {
	...
	if branchSession.Type == api.AT && branchSession.LockKey != "" {
		...
		for _, rowKey := range rowKeys {
			comparisons = append(comparisons, notFound(rowKey))	//如果这些key不存在才执行
		}

		for _, rowKey := range rowKeys {
			rowKeyValue := fmt.Sprintf(LockKeyFormat, branchSession.XID, rowKey)	//lk/gs/aggregationSvc/9169597303674880004/order^^^`order`.`so_master`^^^3742195289
			ops = append(ops, clientv3.OpPut(rowKeyValue, rowKey))
			ops = append(ops, clientv3.OpPut(rowKey, rowKeyValue))	//不存在，则lockKey和rowKey互相赋值
		}
	}
