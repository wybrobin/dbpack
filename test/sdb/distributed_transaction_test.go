...
const (
	//dataSourceName2 = "dksl:123456@tcp(127.0.0.1:13308)/employees?interpolateParams=true&timeout=10s&readTimeout=10s&writeTimeout=10s&parseTime=true&loc=Local&charset=utf8mb4,utf8"
	dataSourceName2 = "dksl:123456@tcp(127.0.0.1:13308)/employees?timeout=10s&readTimeout=10s&writeTimeout=10s&parseTime=true&loc=Local&charset=utf8mb4,utf8"
	phiEmployeeDSN  = `root:123@tcp(127.0.0.1:3306)/employees?timeout=1s&readTimeout=1s&writeTimeout=1s&parseTime=true&loc=Local&charset=utf8mb4,utf8`
...
func (suite *_DistributedTransactionSuite) SetupSuite() {
	...
	var conf = &config.DistributedTransaction{
		...
		EtcdConfig: clientv3.Config{
			Endpoints: []string{"172.27.11.138:2379"/*, "localhost:2378", "localhost:2377"*/},
		},
	}
	...
	if suite.NoErrorf(err, "connection error: %v", err) {
		suite.db2 = db2
		////db2.Exec(insertEmployeeForDT, 1, 100002, "1992-06-04", "jane", "watson", "F", "2013-06-01")
		////db2.Exec(insertDepartmentForDT, 1, 1002, "sales")
		//db2.Exec(insertDeptEmpForDT, 2, 100002, 1002, "2020-01-01", "2022-01-01")
		////db2.Exec(insertSalariesForDT, 1, 100002, 8000, "2020-01-01", "2025-01-01")
		////
...
//			result, err = tx.Exec(deleteSql, 100001, 1001)
...
//			result, err = tx.Exec(updateSql, 20000, 100001)
...
//				rows, err := suite.db.Query(checkDeptEmp, 100001, 1001)
...
//				rows, err = suite.db.Query(checkSalaries, 100001, 20000)
...
func (suite *_DistributedTransactionSuite) TestDistributedTransactionQueryRequest() {
	...
	if suite.NoErrorf(err, "begin global transaction error: %v", err) {
		...
		if suite.NoErrorf(err, "begin tx error: %v", err) {
			...
			if suite.NoErrorf(err, "commit local transaction error: %v", err) {
				status, err := transactionManager.Rollback(context.Background(), xid)
				//status, err := transactionManager.Commit(context.Background(), xid)
				if suite.NoErrorf(err, "rollback global transaction err: %v", err) {
					suite.Equal(api.Rollbacking, status)
				}
				time.Sleep(5 * time.Second)
				...
				checkDeptEmp := `SELECT 1 FROM dept_emp WHERE emp_no = ? and dept_no = ?`
				rows, err := suite.db2.Query(checkDeptEmp, 100002, 1002)
				if suite.NoErrorf(err, "check dept_emp error: %v", err) {
					var exists int
					if rows.Next() {
						err := rows.Scan(&exists)
				...
				checkSalaries := `SELECT 1 FROM salaries WHERE emp_no = ? and salary = ?`
				rows, err = suite.db2.Query(checkSalaries, 100002, 20000)
				if suite.NoErrorf(err, "check salaries error: %v", err) {
					var exists int
					if rows.Next() {
						err := rows.Scan(&exists)
